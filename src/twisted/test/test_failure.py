...
def getDivisionFailure(
    ...
    try:
        ...
        f = failure.Failure(exc_value, exc_type, exc_tb, captureVars)
    return f
...
class FailureTests(SynchronousTestCase):
    ...
    def assertDefaultTraceback(self, captureVars: bool = False) -> None:
        ...
        for method, filename, lineno, localVars, globalVars in f.frames:
            ...
            stack += "    {}\n".format(getline(filename, lineno).strip())
    ...
    def _getInnermostFrameLine(self, f: failure.Failure) -> str:
        try:
            f.raiseException()
        except ZeroDivisionError:
            tb = traceback.extract_tb(sys.exc_info()[2])
            return cast(str, tb[-1][-1])
        else:
...
class FindFailureTests(SynchronousTestCase):
    ...
    def test_findFailure(self) -> None:
        ...
        try:
            ...
            self.fail(  # type: ignore[unreachable]
                "No exception raised from raiseException!?"
            )
...
class FormattableTracebackTests(SynchronousTestCase):
    ...
    def test_singleFrame(self) -> None:
        ...
        self.assertEqual(
            traceback.extract_tb(cast(TracebackType, tb)),
            [FrameSummary("filename.py", 123, "method")],
        )
    ...
    def test_manyFrames(self) -> None:
        ...
        self.assertEqual(
            traceback.extract_tb(cast(TracebackType, tb)),
            [
                FrameSummary("filename.py", 123, "method1"),
                FrameSummary("filename.py", 235, "method2"),
            ],
        )

        # We should also be able to extract_stack on it
        self.assertEqual(
            traceback.extract_stack(cast(FrameType, tb.tb_frame)),
            [
                FrameSummary("filename.py", 7, "caller1"),
                FrameSummary("filename.py", 8, "caller2"),
                FrameSummary("filename.py", 123, "method1"),
            ],
        )
...
class FrameAttributesTests(SynchronousTestCase):
    ...
    def test_fakeFrameAttributes(self) -> None:
        ...
        """
        frame = failure._Frame(("dummyname", "dummyfilename", 0, (), ()), None)
...
class DebugModeTests(SynchronousTestCase):
    ...
    def setUp(self) -> None:
        """
        Override L{pdb.post_mortem} so we can verify that it has been called.
        """
        ...
        def restore() -> None:
            ...
            failure.Failure.__init__ = origInit  # type: ignore[assignment]

        self.addCleanup(restore)

        self.result: List[Optional[TracebackType]] = []

        def captureTraceback(traceback: Optional[TracebackType] = None) -> None:
            self.result.append(traceback)

        pdb.post_mortem = captureTraceback  # type: ignore[assignment]
...
class ExtendedGeneratorTests(SynchronousTestCase):
    ...
    def _throwIntoGenerator(self, f: failure.Failure, g: Generator) -> None:
        try:
