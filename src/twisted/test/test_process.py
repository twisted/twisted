...
class UtilityProcessProtocol(protocol.ProcessProtocol):
    ...
    def run(cls, reactor, argv, env):
        ...
        reactor.spawnProcess(
            self, pyExe, [pyExe, b"-u", b"-m", self.programName] + argv, env=env
        )
        return self
...
class PosixProcessBase(object):
    ...
    if runtime.platform.isMacOSX():
        test_executionError.skip = "Test is flaky from a Darwin bug. See #8840."
...
class Win32UnicodeEnvironmentTests(unittest.TestCase):
    ...
    def test_encodableUnicodeEnvironment(self):
        ...
        def gotEnvironment(environ):
            self.assertEqual(
                environ[self.goodKey.encode('ascii')], self.goodValue.encode('ascii')
            )
