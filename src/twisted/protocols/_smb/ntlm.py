...
class NTLMManager(object):
    ...
    def ntlm_negotiate(self, data):
        ...
        if 'NegotiateOemDomainSupplied' in flags and neg.domain_len > 0:
            self.client_domain = \
            self.token[neg.domain_len:neg.domain_len+neg.domain_offset].decode('utf-16le')
        ...
        if 'NegotiateOemWorkstationSupplied' in flags and neg.workstation_len > 0:
            self.workstation = \
            self.token[neg.workstation_len:neg.workstation_len+neg.workstation_offset].decode('utf-16le')
