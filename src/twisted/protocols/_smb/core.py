...
COMMANDS=[
    ...
    ('logoff',
     ...
     base.nstruct("size:H reserved:H")),    
    ('tree_connect',
     base.nstruct("size:H reserved:H offset:H buflen:H"),
...
class SMBConnection(base.SMBPacketReceiver):
        ...
        try:
            ...
            try:   
                if hasattr(self, func) and req_type:
                   ...
                   log.error("command '%s' not implemented" % COMMANDS[self.hdr_command][0])
                   self.error_response(STATUS_NOT_IMPLEMENTED)
    ...
    def smb_negotiate(self, req, resp):
        ...
        dialects = struct.unpack("<%dH" % req.dialect_count, 
           req.buffer[req.size-len(req):req.size-len(req)+(req.dialect_count*2)])
        self.signing_enabled = (req.security_mode & NEGOTIATE_SIGNING_ENABLED) > 0
        # by spec this should never be false
        self.signing_required = (req.security_mode & NEGOTIATE_SIGNING_REQUIRED) > 0
        log.debug("NEGOTIATE")
        log.debug("---------")
    ...
    def smb_session_setup(self, req, resp):
        blob = req.buffer[req.offset-len(req)-64:req.offset-len(req)-64+req.buflen]
    ...
    def smb_tree_connect(self, req, resp):
        if self.avatar is None:
            ...
            return
        assert req.size == 9
        path = req.buffer[req.offset-len(req)-64:req.offset-len(req)-64+req.buflen]
