...
'''
    ...
    def test_lookupAddress(self):
        ...
        def resolved(results):
            ...
            self.assertEqual(
                (
                    RRHeader(
                        b"multiple", A, IN, self.ttl, Record_A("1.1.1.3", self.ttl)
                    ),
                    RRHeader(
                        b"multiple", A, IN, self.ttl, Record_A("1.1.1.4", self.ttl)
                    ),
                ),
                answers,
            )
    ...
    def test_query(self):
        ...
        d.addCallback(
            lambda x: self.assertEqual(x[0][0].payload.dottedQuad(), '1.1.1.1')
        )
        return d
