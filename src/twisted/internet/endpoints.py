...
class _SimpleHostnameResolver(object):
    ...
    def _ebLog(self, error, hostName):
        """
        Log a hostname resolution failure generated by
        L{_nameResolution}.

        @param error: The hostname resolution failure.
        @type error: L{Failure}

        @param hostName: The hostname that failed to resolve.
        @type hostName: L{Failure}
        """
    ...
    def _deliver(self, gairesult, receiver):
        """
        Deliver C{getaddrinfo} results to a resolution receiver.

        @param gairesult: The iterable of C{getaddrinfo} results to
            deliver.

        @param receiver: The resolution receiver.
        @type receiver: L{IResolutionReceiver}
        """
        for family, socktype, proto, canonname, sockaddr in gairesult:
            if family == socket.AF_INET6:
                receiver.addressResolved(IPv6Address('TCP', *sockaddr))
            elif family == socket.AF_INET:
                receiver.addressResolved(IPv4Address('TCP', *sockaddr))
...
class HostnameEndpoint(object):
    ...
    def _getNameResolverAndMaybeWarn(self, reactor):
        ...
        if not IReactorPluggableNameResolver.providedBy(reactor):
            ...
            return _SimpleHostnameResolver(self._nameResolution)
