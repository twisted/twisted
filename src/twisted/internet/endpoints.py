...
def _redirectfy_https(target_protocol):
    """
    Wrap the protocol and if first received data is HTTP redirect with a
    Host header, redirect to HTTPS
    """
    ...
    def first_ssl_receive(data):
        """
        Called when initial connection data is received over SSL.
        ...
        if not host:
            ...
            return protocol_receive(data)

        target_protocol.transport.write(
        ...
        target_protocol.transport.loseConnection()
        return b""

    target_protocol.dataReceived = first_ssl_receive


if TLSMemoryBIOFactory:

    class HTTPSMemoryBIOFactory(TLSMemoryBIOFactory):
        def buildProtocol(self, addr):
            protocol = self.protocol(self, self.wrappedFactory.buildProtocol(addr))
            _redirectfy_https(protocol)
            return protocol


class HTTPSServerEndpoint(SSL4ServerEndpoint):
    """
    HTTPS secured TCP server endpoint with an IPv4 configuration that
    redirect HTTP request to HTTPS.
    """

    def listen(self, protocolFactory):
        """
        Implement L{IStreamServerEndpoint.listen} to listen for HTTPS on a
        TCP socket.
        """
        ...
        def listen_https():
            """
            Wrapper for catching all errors into a deferred.
            """
            tlsFactory = HTTPSMemoryBIOFactory(
                self._sslContextFactory, False, protocolFactory
            )
            port = self._reactor.listenTCP(
                self._port, tlsFactory, backlog=self._backlog, interface=self._interface
            )
            port._type = "TLS"
            return port

        return defer.execute(listen_https)
