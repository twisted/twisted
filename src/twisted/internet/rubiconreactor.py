...
from rubicon.objc.core_foundation import cf


from ctypes import util, cdll, CFUNCTYPE
from ctypes import *

cfn = cdll.LoadLibrary(util.find_library('CFNetwork'))
...
CFSocketCallBack = CFUNCTYPE(c_void_p, POINTER(__CFSocket), c_int, c_void_p, c_void_p, c_void_p)

CFSocketCreateWithNative = cfn.CFSocketCreateWithNative

CFSocketCreateWithNative.restype = POINTER(__CFSocket)
CFSocketCreateWithNative.argtypes = [c_void_p, c_int, c_ulong, CFSocketCallBack, POINTER(CFSocketContext)]
...
class CFReactor(PosixReactorBase):
    ...
    def _watchFD(self, fd, descr, flag):
        ...
        if fd in self._fdmap:
            ...
            objcCtx = CFSocketContext(info=ctx)

            cfs = CFSocketCreateWithNative(
                kCFAllocatorDefault, fd,
                kCFSocketReadCallBack | kCFSocketWriteCallBack |
                kCFSocketConnectCallBack,
                CFSocketCallBack(self._socketCallback), byref(objcCtx)
            )
