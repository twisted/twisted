...
class ClientControlSequencesTests(unittest.TestCase, MockMixin):
    ...
    def testModes(self):
        self.parser.dataReceived(
            b"\x1b["
            + b';'.join(map(intToBytes, [modes.KAM, modes.IRM, modes.LNM]))
            + b"h"
        )
        self.parser.dataReceived(
            b"\x1b["
...
class ServerProtocolOutputTests(unittest.TestCase):
    ...
    def test_setModes(self):
        ...
        self.assertEqual(
            ...
            self.CSI + b';'.join(map(intToBytes, modesToSet)) + CSFinalByte.SM.value,
        )
    ...
    def test_resetModes(self):
        ...
        self.assertEqual(
            self.transport.value(),
            self.CSI + b';'.join(map(intToBytes, modesToSet)) + CSFinalByte.RM.value,
        )
...
class DeprecationsTests(unittest.TestCase):
    ...
    def test_colors(self):
        ...
        self.ensureDeprecated(
            "twisted.conch.insults.colors was deprecated "
    ...
    def test_client(self):
        ...
        self.ensureDeprecated(
            "twisted.conch.insults.client was deprecated "
            "in Twisted 10.1.0: Please use "
