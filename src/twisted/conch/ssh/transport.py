...
from twisted.conch.ssh.common import NS, getNS, MP, getMP, _MPpow, ffs, int_from_bytes
...
class SSHTransportBase(protocol.Protocol):
    ...
    def dispatchMessage(self, messageNum, payload):
        ...
        if messageNum < 50 and messageNum in messages:
            ...
            log.callWithLogger(
                self.service, self.service.packetReceived, messageNum, payload
            )
        else:
    ...
    def ssh_KEXINIT(self, packet):
        ...
        log.msg(
            'outgoing: %r %r %r'
            % (
                self.nextEncryptions.outCipType,
...
class SSHServerTransport(SSHTransportBase):
    ...
    def _ssh_KEX_ECDH_INIT(self, packet):
        ...
        sharedSecret = MP(
            int(binascii.hexlify(ecPriv.exchange(ec.ECDH(), theirECPub)), 16)
        )
...
class SSHClientTransport(SSHTransportBase):
    ...
    def _ssh_KEX_ECDH_REPLY(self, packet):
        ...
        def _continue_KEX_ECDH_REPLY(ignored, hostKey, pubKey, signature):
            ...
            sharedSecret = MP(
                int(binascii.hexlify(self.ecPriv.exchange(ec.ECDH(), theirECPub)), 16)
            )
