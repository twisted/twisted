...
def gotRooms(rooms):
    print "got Rooms.", rooms[0]

    for room in rooms:
        print "  ROOM:", room
        if hasattr(room,"furniture"):
            for child in room.furniture:
                print "   ", child
                if hasattr(child, "lamps"):
                    for lamp in child.lamps:
                        print "      ", lamp
        if hasattr(room, "rugs"):
            for child in room.rugs:
                print "   ", child
        if room.roomId == 10:
...
def kickOffTests(ignoredResult=0):
    global manager
    manager = row.DBReflector(dbpool, [RoomRow, FurnitureRow, RugRow, LampRow], runTests)
