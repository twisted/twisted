...
def str2time(s):
    """
    Parse a string description of an interval into an integer number of
    seconds.
...
class RRHeader(tputil.FancyEqMixin):
    ...
    @ivar auth: A C{bool} indicating whether this C{RRHeader} was parsed from
...
class Record_SOA(tputil.FancyEqMixin, tputil.FancyStrMixin):
    ...
    """
    fancybasename = 'SOA'
    compareAttributes = ('serial', 'mname', 'rname', 'refresh', 'expire',
                         'retry', 'minimum', 'ttl')
    showAttributes = (('mname', 'mname', '%s'), ('rname', 'rname', '%s'),
...
class Message(tputil.FancyEqMixin):
    ...
    def toStr(self):
        """
        Encode this L{Message} into a byte string in the format described by
        RFC 1035.
...
class _EDNSMessage(tputil.FancyEqMixin, object):
    ...
    def _toMessage(self):
        ...
        the I{EDNS} specific attributes and options will be appended to the
        list of C{additional} records.
    ...
    def _fromMessage(cls, message):
        ...
        The C{extendedRCODE} will be combined with C{message.rCode} and
        assigned to C{self.rCode}.
        ...
        newMessage = cls(
            ...
            # Default to None, it will be updated later when the OPT records
            # are parsed.
            ednsVersion=None,
            dnssecOK=False,
