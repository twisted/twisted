...
def bitstring(val, lpad=0, rpad=0):
    """
    Convert an integer value to binary string without the C{'0b'}
    prefix and add left and right C{'0'} padding if required.
    """
    return bin(val)[2:].rjust(lpad, '0').ljust(rpad, '0')



def assertBitValue(testCase, expectedValue, testBytes, offset, extent=1):
    x, i = testBytes, 0
    while x:
        x >>= 1
        i += 1

    extractedValue = (testBytes & (2 ** (i-offset+1) - 1))

    x, i = extractedValue, 0
    while x:
        x >>= 1
        i += 1

    extractedValue >>= i-extent
    testCase.assertEqual(
        expectedValue,
        extractedValue)
...
def assertBitValue2(testCase, expectedValue, testBytes, offset, extent=1):
    """
    Assert that a range of bits has the same binary value as the
    supplied C{expectedValue} (network byte order).

    @param testCase: The L{TestCase} on which to call assertion methods.
    @param expectedValue: The expected integer value of the extracted bits.
    @param testBytes: The bytes from which a bit range will be extracted.
    @param offset: The bit offset from which to begin extracting bits.
    @param extent: The number of bits to extract from the offset point.
    """
    bitOffset = offset
    bitExtent = extent
    assert expectedValue < 2 ** bitExtent, (
        'Expected value must be <%s (2 ** %s). Got %s' % (
            2**bitExtent, bitExtent, bin(expectedValue)))

    byteOffset, remainingBitOffset = divmod(bitOffset, 8)
    byteExtent, remainingBitExtent = divmod(bitExtent, 8)
    if remainingBitExtent > 0:
        byteExtent += 2

    testBytesExtracted = ''
    for b in testBytes[byteOffset:byteOffset+byteExtent]:
        testBytesExtracted += bitstring(ord(b), lpad=8)

#    import pdb;pdb.set_trace()

    if remainingBitOffset:
        testBytesExtracted = testBytesExtracted[remainingBitOffset:]
    if remainingBitExtent:
        testBytesExtracted = testBytesExtracted[0:remainingBitExtent]

    testCase.assertEqual(
        bitstring(expectedValue, lpad=extent),
        testBytesExtracted)
...
class EDNSMessageStandardEncodingTests(unittest.SynchronousTestCase):
    ...
    def test_idEncode(self):
        """
        The id flag is encoded in the first 16 bits of the message.
        """
        assertBitValue(
            self,
            65535,
            dns._EDNSMessage(id=65535).toStr(),
            offset=0, extent=16)


    def test_answerEncode(self):
        """
        The answer flag is encoded in bit16 of the message.
        """
        assertBitValue(
            self,
            1,
            dns._EDNSMessage(answer=True).toStr(),
            offset=16)


    def test_opCodeEncode(self):
        """
        The opcode is encoded in the 4 bits starting at bit 17.
        """
        assertBitValue(
            self,
            4,
            dns._EDNSMessage(opCode=4).toStr(),
            offset=17, extent=4)


    def test_authEncode(self):
        """
        The auth flag is encoded in bit 21 of the message.
        """
        assertBitValue(
            self,
            1,
            dns._EDNSMessage(auth=True).toStr(),
            offset=21)


    def test_truncEncode(self):
        """
        The trunc flag is encoded in bit 22 of the message.
        """
        assertBitValue(
            self,
            1,
            dns._EDNSMessage(trunc=True).toStr(),
            offset=22)


    def test_recDesEncode(self):
        """
        The recDes flag is encoded in bit 23 of the message.
        """
        assertBitValue(
            self,
            1,
            dns._EDNSMessage(recDes=True).toStr(),
            offset=23)


    def test_recAvEncode(self):
        """
        The recAv flag is encoded in bit 24 of the message.
        """
        assertBitValue(
            self,
            1,
            dns._EDNSMessage(recAv=True).toStr(),
            offset=24)


    def test_zEncode(self):
        """
        Bits 25-28 are reserve and default to 0.
        """
        assertBitValue(
            self,
            0,
            dns._EDNSMessage().toStr(),
            offset=25, extent=3)


    def test_rcodeEncode(self):
        """
        The rCode is encoded in the 4bits starting at bit 28 of the
        message.
        """
        assertBitValue(
            self,
            15,
            dns._EDNSMessage(rCode=15).toStr(),
            offset=28, extent=4)


    def test_bincompare(self):
        expectedValue = 0b111
        testBytes = 0b10001110
        offset = 0
        extent = 5
        ...
        self.assertEqual(
            [bin(testBytes), bin(expectedValue), bin(extractedValue)],
            [])
