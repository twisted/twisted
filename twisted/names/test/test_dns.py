...
class DNSProtocolTestCase(unittest.TestCase):
    ...
    def test_toListReturnValueIsList(self):
        """
        L{dns.Message.toList} returns a list.
        """
        m = dns.Message()
        m.id = 12345
    ...
    def test_encodeParts(self):
        """
        The result from L{dns.Message.encodeParts} method is the same as the
        result from the encode method.
        """
        answers = []
        for i in range(20):
            answers.append(dns.RRHeader(
                    payload=dns.Record_A(address='1.2.3.%s' % (i,))))
        ...
        self.assertEquals(parts[0], strio.getvalue())


    def test_encodePartsWithCustomMaxSize(self):
      """
      L{dns.Message.encodeParts} returns multiple parts if a C{maxPartSize}
      value is provided which is smaller than the total size of all the answers.
      """
      answers = []
      for i in range(100):
          for j in range(80):
            answers.append(dns.RRHeader(
                    payload=dns.Record_A(address='1.2.%s.%s' % (i, j))))
...
class EqualityTests(unittest.TestCase):
    ...
    def test_spf(self):
        ...
        records, so they are equal if and only if they have the same data and
        ttl.
        """
        # Vary the length of the data
        self._equalityTest(
