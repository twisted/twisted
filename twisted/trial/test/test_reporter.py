...
import errno, sys, os, re, StringIO, warnings
...
def getWarnings(f, *a, **kw):
    """
    Return a list of all warnings generated by running C{f} with the given
    arguments. Will not catch exceptions that C{f} might raise.

    @return: A list of (message, category, filename, lineno[, file]).
    """
    # XXX - this should probably be somewhere more general
    log = []
    def showwarning(*a):
        log.append(a)
    warnings.showwarning, _old = showwarning, warnings.showwarning
    try:
        f(*a, **kw)
    finally:
...
class TestGetWarnings(unittest.TestCase):
    """
    Make sure the getWarnings method (defined above) actually works.
    """

    warningText = "Snake oh its a snake"
    
    def silent(self):
        """
        Simple method that does not generate any warnings.
        """
        pass

    def warning(self):
        """
        Simple method that generates a warning.
        """
        warnings.warn(self.warningText)

    def test_silent(self):
        """
        Test that we don't get any warnings from methods that don't generate
        warnings. Converse of test_warning.
        """
        self.assertEqual(getWarnings(self.silent), [])

    def test_warning(self):
        """
        Test that we get all warnings from methods that generate warnings.  
        """
        log = getWarnings(self.warning)
        self.assertEqual(str(log[0][0]), self.warningText)
        self.assertEqual(log[0][1], UserWarning)


class TestDirtyReactor(unittest.TestCase):
    """
    The trial script has an option to L{DirtyReactorError}s as warnings, as a
    migration tool for test authors. These tests check the reporting of
    L{DirtyReactorError}s.
    """
    
    def setUp(self):
        self.dirtyError = failure.Failure(util.DirtyReactorError('foo'))
        self.pendingError = failure.Failure(util.PendingTimedCallsError('bar'))
        self.output = StringIO.StringIO()
        self.test = TestDirtyReactor('test_errorByDefault')

    def test_errorByDefault(self):
        """
        Test that C{DirtyReactorError}s are reported as errors with the default
        Reporter options.
        """
        result = reporter.Reporter(stream=self.output)
        result.addError(self.test, self.dirtyError)
        result.addError(self.test, self.pendingError)
        self.assertEqual(len(result.errors), 2)
        self.assertEqual(result.errors[0][1], self.dirtyError)
        self.assertEqual(result.errors[1][1], self.pendingError)

    def test_warningsEnabled(self):
        """
        Test that C{DirtyReactorErrors}s are reported as warnings when the
        C{uncleanWarnings} optional argument is C{True}.
        """
        result = reporter.Reporter(stream=self.output, uncleanWarnings=True)
        warns = getWarnings(result.addError, self.test, self.dirtyError)
        self.assertEqual(len(warns), 1)
        self.assertEqual(str(warns[0][0]), self.dirtyError.getErrorMessage())
        warns = getWarnings(result.addError, self.test, self.pendingError)
        self.assertEqual(len(warns), 1)
        self.assertEqual(str(warns[0][0]), self.pendingError.getErrorMessage())

    def test_warningsMaskErrors(self):
        """
        Test that C{DirtyReactorErrors}s are I{not} reported as errors when
        the C{uncleanWarnings} optional argument is C{True}.
        """
        result = reporter.Reporter(stream=self.output, uncleanWarnings=True)
        # don't print warnings
        getWarnings(result.addError, self.test, self.dirtyError)
