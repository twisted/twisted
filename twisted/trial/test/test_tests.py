...
class IterateTestsMixin(object):
    """
    L{iterateTests} returns a list of all test cases in a test suite or test
    case.
    """
    if _PY3:
        skip = _PY3PORTNEEDED
    ...
    def test_iterateTestCase(self):
        """
        L{iterateTests} on a single test case returns a list containing that
        test case.
        """
        test = self.TestCase()
        self.assertEqual([test], list(unittest.iterateTests(test)))


    def test_iterateSingletonTestSuite(self):
        """
        L{iterateTests} on a test suite that contains a single test case
        returns a list containing that test case.
        """
        test = self.TestCase()
        suite = runner.TestSuite([test])
        self.assertEqual([test], list(unittest.iterateTests(suite)))


    def test_iterateNestedTestSuite(self):
        """
        L{iterateTests} returns tests that are in nested test suites.
        """
        test = self.TestCase()
    ...
    def test_iterateIsLeftToRightDepthFirst(self):
        """
        L{iterateTests} returns tests in left-to-right, depth-first order.
        """
        test = self.TestCase()
        suite = runner.TestSuite([runner.TestSuite([test]), self])
        self.assertEqual([test, self], list(unittest.iterateTests(suite)))
