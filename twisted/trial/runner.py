...
#
#  The method runner wraps the method, locates the method's class and modules so that the setUp{Module,
#  Class, } can be run for that test. 
#
#  ------
#
    ...
    def run(self, seed=None):
        ...
        try:
            ...
            for test in tests:
                ...
                try:
                    tr.runTests(self.reporter, self._janitor)
                except KeyboardInterrupt:
                    log.msg(iface=ITrialDebug, kbd="KEYBOARD INTERRUPT")
                    _bail()
                    raise
    ...
    def runTests(self, reporter, janitor):
        ...
        try:
            ...
            for mname in self.methodNames:
                ...
                log.msg("--> %s.%s.%s <--" % (testMethod.module,
