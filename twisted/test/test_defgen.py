...
# with slightly different syntax for the DefGenTests below.
...
class BaseDefgenTests:
    ...
    deferredGenerator and defgen based generators. The subclasses
    DeferredGeneratorTests and DefGenTests each provide the actual
...
newdefgen_tests = '''
from twisted.internet.defer import defgen, returnValue

class DefGenTests(BaseDefgenTests, unittest.TestCase):
    # First provide all the generator impls necessary for BaseDefgenTests
    ...
    def _genBasics(self):
        ...
        try:
            ...
            self.assertEquals(str(e), 'OMG')
        returnValue("WOOSH")
    _genBasics = defgen(_genBasics)

    def _genBuggy(self):
        yield getThing()
        1/0
    _genBuggy = defgen(_genBuggy)
    

    def _genNothing(self):
        if 0: yield 1
    _genNothing = defgen(_genNothing)
    ...
    def _genHandledTerminalFailure(self):
        try:
            ...
            pass
    _genHandledTerminalFailure = defgen(_genHandledTerminalFailure)
    ...
    def _genHandledTerminalAsyncFailure(self, d):
        try:
            ...
            pass
    _genHandledTerminalAsyncFailure = defgen(_genHandledTerminalAsyncFailure)
    ...
    def _genStackUsage(self):
        ...
        returnValue(0)
    _genStackUsage = defgen(_genStackUsage)
    ...
    def _genStackUsage2(self):
        for x in range(5000):
            ...
            yield 1
        returnValue(0)
    _genStackUsage2 = defgen(_genStackUsage2)

    # Tests unique to defgen
