...
class FTPShellTestCase(unittest.TestCase, IFTPShellTestCase):
    """
    ...
    def createFile(self, path):
        ...
        Create a file named C{path} with some content.
        """
        return self.root.child(path).setContent('wobble\n')



class TestConsumer(object):
    """
    A simple consumer for tests.
    """
...
class IReadWriteTestCase:
    ...
    def test_read(self):
        """
        Test L{ftp.IReadFile}.
        """
        content = 'wobble\n'
        reader = self.getFileReader(content)
        consumer = TestConsumer()
        def cb(res):
            self.assertEquals("".join(consumer.buffer), content)
        return reader.send(consumer).addCallback(cb)
    ...
    def test_write(self):
        ...
        def cb(consumer):
            producer = TestProducer('elbbow\n', consumer)
            consumer.registerProducer(None, True)
            producer.start()
            consumer.unregisterProducer()
            self.assertEquals(self.getFileContent(), 'elbbow\n')
        return writer.receive().addCallback(cb)



class FTPReadWriteTestCase(unittest.TestCase, IReadWriteTestCase):
    """
    Tests for C{ftp._FileReader} and C{ftp._FileWriter}.
    """
    ...
    def getFileReader(self, content):
        """
        Return a C{ftp._FileReader} instance with an opened file for reading.
        """
        f = open(self.filename, "wb")
        f.write(content)
        f.close()
        f = open(self.filename, "rb")
        return ftp._FileReader(f)


    def getFileWriter(self):
        """
        Return a C{ftp._FileWriter} instance with an opened file for writing.
        """
        f = open(self.filename, "wb")
        return ftp._FileWriter(f)
