...
class AppProfilingTestCase(unittest.TestCase):
    ...
    def test_profile(self):
        """
        L{app.AppProfiler.runWithProfile} should call the C{run} method of the
    ...
    def test_profileSaveStats(self):
        ...
        L{app.AppProfiler.runWithProfile} should save the raw stats object
    ...
    def test_withoutProfile(self):
        """
        When the C{profile} module is not present,
        L{app.AppProfiler.runWithProfile} should raise a C{SystemExit}
        exception.
        """
    ...
    def test_hotshot(self):
        """
        L{app.AppProfiler.runWithHotshot} should call the C{run} method of the
        reactor and save profile data in the specified file.
        """
        config = twistd.ServerOptions()
        config["profile"] = self.mktemp()
    ...
    def test_hotshotSaveStats(self):
        """
        With the C{savestats} option specified,
        L{app.AppProfiler.runWithHotshot} should save the raw stats object
        instead of a summary output.
        """
        config = twistd.ServerOptions()
        config["profile"] = self.mktemp()
    ...
    def test_withoutHotshot(self):
        """
        When the C{hotshot} module is not present,
        L{app.AppProfiler.runWithHotshot} should raise a C{SystemExit}
