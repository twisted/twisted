...
class DeferredTestCase(unittest.TestCase):
    ...
    def test_asynchronousImplicitErrorChain(self):
        ...
        If C{a} has no result and is returned from a callback on C{b} then
        when C{a} fails, C{b}'s result becomes the L{Failure} that was C{a}'s
        result , the result of C{a} becomes C{None} and no unhandled error is
        logged when it is garbage collected.
        """
        first = defer.Deferred()
        second = defer.Deferred()
        second.addCallback(lambda ign: first)
    ...
    def test_doubleAsynchronousImplicitChaining(self):
        ...
        third.addCallback(self.assertEquals, result)
        return third
    ...
    def test_nestedAsynchronousChainedDeferredsWithExtraCallbacks(self):
        ...
        # A Deferred returned wrapped in a 'gatherResults' in the inner
        # callback.
        inner = defer.Deferred()
        ...
        def cb(result):
            ...
            def firstCallback(ignored):
                ...
                # Return a Deferred that definitely has not fired yet wrapped
                # in a gatherResults so we can fire the Deferreds out of order
                # and see how gatherResults affects the stack.
    ...
    def test_repr(self):
        ...
        """
        d = defer.Deferred()
        self.assertEquals(repr(d), '<Deferred at %s>' % (hex(unsignedID(d))))
    ...
    def test_reprWithChaining(self):
        ...
        b.chainDeferred(a)
        self.assertEquals(
            repr(a), "<Deferred at %s waiting on %r>" % (
                hex(unsignedID(a)), b))
