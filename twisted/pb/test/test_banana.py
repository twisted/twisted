...
from twisted.pb import banana, slicer, schema, tokens, debug
...
try:
    ...
    sets = None
have_bool = hasattr(types, "BooleanType")
...
class TestBananaMixin:
    ...
    def looptest(self, obj):
        d = self.loop(obj)
        d.addCallback(self._looptest_1, obj)
        return d
    def _looptest_1(self, obj2, obj):
...
class InboundByteStream2(TestBananaMixin, unittest.TestCase):
    ...
    def testConstrainedBool(self):
        if not have_bool:
            raise unittest.SkipTest("python2.2 does not have booleans")
...
class ThereAndBackAgain(TestBananaMixin, unittest.TestCase):
    ...
    def test_set(self):
        if sets is None:
            raise unittest.SkipTest("python2.2 does not have the sets module")
        d = self.looptest(sets.Set([1,2]))
        d.addCallback(lambda res: self.looptest(sets.ImmutableSet([1,2])))
        return d

    def test_bool(self):
        # this test can be done on python2.2 anyway
