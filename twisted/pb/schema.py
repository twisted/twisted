...
def makeConstraint(t):
    ...
    if hasattr(types, "BooleanType"):
        # TODO: when we drop 2.2 support, make this unconditional
        map[types.BooleanType] = BooleanConstraint()
    else:
        # in 2.2, type(True) is types.IntType, so 2.2 senders will never emit
        # ("boolean"..) sequences. Also, 'bool' is just a function.
        map[bool] = IntegerConstraint()
    c = map.get(t, None)
    if c:
        return c
    try:
