...
class StdioClient(basic.LineReceiver):
    ...
    def cmd_PUT(self, rest):
        """
        Do an upload request for a single local file or globing expression.

        @param rest: Request line after the put command.
        @type  rest: C{str}
...
"""
    ...
    def _getFilename(self, line):
        """
        Parse line and return first filename together with the remaining line.

        @return: Tupple with filename and rest. Return empty values when
