...
class POP3Client(basic.LineOnlyReceiver, policies.TimeoutMixin):
    ...
    @type serverChallenge: L{bytes} or C{type(None)}
    ...
    @type _capCache: C{type(None)} or L{dict} mapping L{bytes}
        to L{list} of L{bytes} and/or L{bytes} to C{type(None)}
    ...
    @type _blockedQueue: C{type(None)} or L{list} of 3-L{tuple}
    ...
    @type _greetingError: L{bytes} or C{type(None)}
    ...
    def _blocked(self, f, *a):
        ...
        @rtype: C{type(None)} or L{Deferred <defer.Deferred>}
    ...
    def sendLong(self, cmd, args, consumer, xform):
        ...
        @type xform: C{type(None)} or callable that takes
    ...
    def startTLS(self, contextFactory=None):
        ...
        @type contextFactory: C{type(None)} or
        ...
        @rtype: L{Deferred <defer.Deferred>} which successfully results in
            ...
            C{type(None)} or fails with L{TLSError}
    ...
    def _startTLS(self, caps, contextFactory, tls):
        ...
        @rtype: L{Deferred <defer.Deferred>} which successfully triggers with
            L{dict} mapping L{bytes} to L{list} of L{bytes} and/or L{bytes} to
            C{type(None)} or fails with L{TLSNotSupportedError}
    ...
    def _consumeOrSetItem(self, cmd, args, consumer, xform):
        ...
        @type consumer: C{type(None)} or callable that takes
            L{object}
        @param consumer: C{None} or a function that consumes the output from
            the transform function.

        @type xform: C{type(None)}, callable that takes
    ...
    def retrieve(self, index, consumer=None, lines=None):
        ...
        @type lines: C{type(None)} or L{int}
