...
init_c_dir(void)
{
	...
	PyDirObject_Error = PyErr_NewException("_c_dir.error", PyDirObject_Error, NULL);
	...
	m = Py_InitModule3("_c_dir", dir_functions, dir__doc__);
	if (m == NULL)
		return;
	...
	for (i = 0; dtvalues[i].name != NULL; ++i) {
		if (!(dtval = PyInt_FromLong(dtvalues[i].value)))
			return;
		if (PyDict_SetItemString(d, dtvalues[i].name, dtval))
			return;
	}
