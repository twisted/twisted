...
# written by jml and countless contributors
...
class ReporterPlugins(object):
    ...
    def load(self):
        ...
        sio = StringIO()
        sys.stdout.write("loading reporter plugins...."); sys.stdout.flush()
        ...
        try:
            ...
            for p in self.plugins:
                self.origout.write(".");self.origout.flush()
...
##     def opt_cleanup(self, arg):
##         """specify cleanups the test framework should perform after each test run
##         useful options are ''
##         """
##         self['cleanup'] = arg
...
def _doProfilingRun(config, suite):
    ...
    prof = profile.Profile(lineevents=1)
    try:
