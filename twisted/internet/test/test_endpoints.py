...
class TLSClientStringTests(unittest.TestCase):
    """
    Tests for L{twisted.internet.endpoints.clientFromString} which require TLS.
    """
    ...
    def test_ssl(self):
        ...
        string, and raises a deprecation warning.
        """
        reactor = object()
        client = endpoints.clientFromString(
            reactor,
        ...
        self.assertEqual(len(warnings), 1)
        self.assertIn(
            ("twisted.internet.endpoints.SSL4ClientEndpoint was deprecated in "
             "Twisted 15.5.0: Use TLS4ClientEndpoint instead."),
            warnings[0]["message"])


    def test_tls(self):
        """
        When passed an TLS strports description, L{clientFromString} returns a
        ...
        client = endpoints.clientFromString(
            reactor,
            "tls:host=example.net:port=4321:privateKey=%s:"
            "certKey=%s:bindAddress=10.0.0.3:timeout=3:caCertsDir=%s" %
            (escapedPEMPathName, escapedPEMPathName, escapedCAsPathName))
    ...
    def test_tlsPositionalArgs(self):
        """
        When passed an TLS strports description, L{clientFromString} returns a
        L{TLS4ClientEndpoint} instance initialized with the values from the
        string.
        ...
        client = endpoints.clientFromString(
            reactor,
            "tls:example.net:4321:privateKey=%s:"
            "certKey=%s:bindAddress=10.0.0.3:timeout=3:caCertsDir=%s" %
            (escapedPEMPathName, escapedPEMPathName, escapedCAsPathName))
        self.assertIsInstance(client, endpoints.TLS4ClientEndpoint)
        self.assertIs(client._reactor, reactor)
    ...
    def test_tlsWithDefaults(self):
        """
        When passed an TLS strports description without extra arguments,
        L{clientFromString} returns a L{TLS4ClientEndpoint} instance
        whose context factory is initialized with default values.
        """
        reactor = object()
        client = endpoints.clientFromString(reactor, "tls:example.net:4321")
        self.assertIsInstance(client, endpoints.TLS4ClientEndpoint)
        self.assertIs(client._reactor, reactor)
        self.assertEqual(client._host, "example.net")
        self.assertEqual(client._port, 4321)
    ...
    def test_tlsSimple(self):
        """
        When passed an TLS strports description without any extra parameters,
        L{clientFromString} returns a simple non-verifying endpoint that will
        speak TLS.
        """
        reactor = object()
        client = endpoints.clientFromString(
            reactor, "tls:host=simple.example.org:port=4321")
        certOptions = client._sslContextFactory
        self.assertIsInstance(certOptions, CertificateOptions)
