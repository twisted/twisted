...
class InterfacesTestCase(unittest.TestCase):
    ...
    def testExplicitRegistrationOfImplementation(self):
        ...
        o = Sub()
        self.assert_(not components.implements(o, IMultiply))
...
class Adept(components.Adapter):
    __implements__ = IAdept, components.Adapter.__implements__
...
class Elapsed(components.Adapter):
    __implements__ = IElapsed, components.Adapter.__implements__
    def elapsedFunc(self):
        return 1
...
class ComponentizedTestCase(unittest.TestCase):
    ...
    def testComponentized(self):
        c = Compo()
        self.assertEqual(c.getComponent(IAdept).adaptorFunc(), (1, 1))
        self.assertEqual(c.getComponent(IAdept).adaptorFunc(), (2, 2))
        self.assertEqual(IElapsed(IAdept(c)).elapsedFunc(), 1)

    def testPreComponentized(self):
        a = components.PreComponentized()
        a.getComponent = "hello world"
        components.IComponentized(a).addComponent(Elapsed(self), ignoreClass=1)
        self.assertEqual(IElapsed(a).elapsedFunc(), 1)
...
class ComponentMeta(components.Adapter):
    __implements__ = IMeta, components.Adapter.__implements__
...
class Dest1Impl(components.Adapter):
    __implements__ = IIDest1, components.Adapter.__implements__
