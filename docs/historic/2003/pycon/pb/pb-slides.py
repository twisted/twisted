...
lecture = Lecture(
    ...
    Slide(
        ...
        Bullet(
            "What happens to the results that are returned?",
    ...
    Slide(
        "Whither Translucence?",
        Bullet(
            "Not 'Transparent': don't pretend remote objects are really local",
            SubBullet(
                "CORBA (in C) does this, makes remote calls look like local calls"
            ),
            SubBullet("makes it hard to deal with the async nature of RPC"),
        ),
        Bullet(
            "Not 'Opaque': make it easy to deal with the differences",
    ...
    Slide(
        ...
        Bullet(
            HTML(
                '<span class="py-src-identifier">pb.Referenceable</span>: Object which can be accessed by remote systems.'
            ),
            SubBullet(
                HTML(
                    'Defines methods like <span class="py-src-identifier">remote_foo</span> and <span class="py-src-identifier">remote_bar</span> which can be invoked remotely.'
                )
            ),
            SubBullet(
                HTML(
                    'Methods without the <span class="py-src-identifier">remote_</span> prefix are local-only.'
                )
            ),
        ),
        Bullet(
            HTML(
                '<span class="py-src-identifier">pb.RemoteReference</span>: Used by distant program to invoke methods.'
            ),
            SubBullet(
                HTML(
                    'Offers <span class="py-src-identifier">.callRemote()</span> to trigger remote method on a corresponding <span class="py-src-identifier">pb.Referenceable</span>.'
                )
            ),
        ),
    ),
    Raw("Sample code", "<h2>Sample Code</h2>" + server_lore + client_lore),
    # Slide("Simple Demo"),
    ...
    Slide(
        ...
        Bullet(
            "Mutable objects get reference IDs to insure shared references remain shared",
    ...
    Slide(
        ...
        Bullet(
            HTML(
                'User classes inherit from one of the <span class="py-src-identifier">pb.flavor</span> classes'
            )
        ),
        Bullet(
            HTML(
                '<span class="py-src-identifier">pb.Referenceable</span>: methods can be called remotely'
            )
        ),
        Bullet(
            HTML(
                '<span class="py-src-identifier">pb.Copyable</span>: contents are selectively copied'
            )
        ),
        Bullet(
            HTML(
                '<span class="py-src-identifier">pb.Cacheable</span>: contents are copied and kept up to date'
            )
        ),
        Bullet(
            HTML(
                'Classes define <span class="py-src-identifier">.getStateToCopy</span> and other methods to restrict exported state'
            )
        ),
    ),
    Slide(
...
"""
    ...
    Slide(
        "Secure Unjellying",
        Bullet(
            "Pickle has security problems",
            SubBullet("Pickle will import any module the sender requests."),
            SubBullet(
                HTML(
                    '2.3 gave up, removed safety checks like <span class="py-src-identifier">__safe_for_unpickling__</span> .'
                )
            ),
        ),
        Bullet(
            "Jelly attempts to be safe in the face of hostile clients",
            SubBullet("All classes rejected by default"),
            SubBullet(
                HTML(
                    '<span class="py-src-identifier">registerUnjellyable()</span> used to accept safe ones'
                )
            ),
            SubBullet(
                HTML(
                    'Registered classes define <span class="py-src-identifier">.setCopyableState</span> and others to process remote state'
                )
            ),
        ),
    ...
    Slide(
        "Transformation of references in transit",
        Bullet(
            "All referenced objects get turned into their counterparts as they go over the wire"
        ),
        Bullet(
            "References are followed recursively",
            SubBullet(
                "Sending a reference to a tree of objects will cause the whole thing to be transferred"
            ),
            SubBullet("(subject to security restrictions)"),
        ),
        Bullet(
            HTML('<span class="py-src-identifier">pb.flavors</span> get reference ids'),
            SubBullet(
                "They are recognized when they return, transformed into the original reference"
            ),
    ...
    Slide(
        "Perspectives: pb.cred and the Identity/Service model",
        Bullet(
            "A layer to provide common authentication services to Twisted applications"
        ),
        Bullet(
            HTML(
                '<span class="py-src-identifier">Identity</span>: named user accounts with passwords'
            )
        ),
        Bullet(
            HTML(
