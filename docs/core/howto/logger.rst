...
Applications that wish to switch from the old module to the new module without having to change much code may do so my using the :api:`twisted.logger.LegacyLogger <LegacyLogger>` class, which exposes an API that is largely compatible with many uses of the :api:`twisted.python.log <twisted.python.log>` module.
...
…could be modified thusly to stop depending on the older module by simply changing the import line and instantiating a :api:`twisted.logger.LegacyLogger <LegacyLogger>` .
