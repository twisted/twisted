...
a :doc:`separate document <udp>` covering UDP.

    


Your protocol handling class will usually subclass :api:`twisted.internet.protocol.Protocol <twisted.internet.protocol.Protocol>` . Most
...
from :api:`twisted.internet.protocol.Factory <twisted.internet.protocol.Factory>` . The ``buildProtocol`` 
method of the ``Factory`` is used to create
a ``Protocol`` for each new connection.
...
the ``Factory`` does not listen to connections, and in
fact does not know anything about the
network. See :doc:`the endpoints documentation <endpoints>` for more information,
or :api:`twisted.internet.interfaces.IReactorTCP.listenTCP <twisted.internet.interfaces.IReactorTCP.listenTCP>` ,
and the other ``IReactor*.listen*`` APIs for the lower
...
be passed to ``Echo.__init__`` when creating a new
