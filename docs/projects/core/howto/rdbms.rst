...
interfaces -- for this reason, :api:`twisted.enterprise.adbapi <twisted.enterprise.adbapi>` was created. It is
...
- Familiarity with using database interfaces (see `the documentation for DBAPI 2.0 <http://www.python.org/dev/peps/pep-0249/>`_ or this `article <http://www.amk.ca/python/writing/DB-API.html>`_ 
  by Andrew Kuchling)
...
:api:`twisted.enterprise.adbapi <twisted.enterprise.adbapi>` , an
asynchronous wrapper for any `DB-API 2.0 <http://www.python.org/dev/peps/pep-0249/>`_ -compliant module.

    


:api:`twisted.enterprise.adbapi <enterprise.adbapi>` will do
blocking
...
:api:`twisted.enterprise.adbapi.ConnectionPool <adbapi.ConnectionPool>` 
class to manage
a connections for you. This allows :api:`twisted.enterprise.adbapi <enterprise.adbapi>` to use multiple
...
- There is no need to import dbmodule directly. You just
  pass the name to :api:`twisted.enterprise.adbapi.ConnectionPool <adbapi.ConnectionPool>` 's constructor.
- The parameters you would pass to dbmodule.connect are
  passed as extra arguments to :api:`twisted.enterprise.adbapi.ConnectionPool <adbapi.ConnectionPool>` 's constructor.
...
of ``getAge`` . It returns a :api:`twisted.internet.defer.Deferred <twisted.internet.defer.Deferred>` , which allows
arbitrary callbacks to be called upon completion (or upon
failure).  More documentation on Deferred is available :doc:`here <defer>` .
...
The function will be called in the thread with a :api:`twisted.enterprise.adbapi.Transaction <twisted.enterprise.adbapi.Transaction>` ,
...
paramater munging -- ``runQuery(query, params, ...)`` maps directly onto ``cursor.execute(query, params, ...)`` .
